...
#    	printf("%s %s|%s %s %s\n", $route{$noun_VCoreName}, $exit_status, $archive_span_perfdata, $uptime_perfdata);
    ...
    printf("Recording status %s: %s\n", $rec_status, $route{$noun_RecordingState});
    if ($no_archive) {
	    printf("Archive status %s: archive has no records\n", $rec_status);
    } else {
	    printf("Archive status %s: the last event has been recorded %s\n", $arch_status, from_now($dur->seconds(),1));
    }
    printf("Archive %s%s\n", $archive_span_status, duration($archive_span_sec,1));
    printf("Archive from %s\n", $route{$noun_ArchiveEarliestDate});
    printf("Archive till %s\n", $route{$noun_ArchiveMostRecentDate});
    ...
    if (!$o_legacy_vserver) {
    	printf("Licensed Cameras: %d\n", $route{$noun_NumberOfCamerasLicensed});
    }
    printf("%s\n", $vcore_info);
    last;
...
#		printf("%s cameras: %s|enabled=%s healthy=%s faulty=%s\n", $route{$noun_VCoreName}, $exit_status, $cam_total_count, $cam_healthy_count, $cam_faulty_count);
